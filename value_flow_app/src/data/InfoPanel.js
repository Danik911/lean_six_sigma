  ...
  switch(nodeType) {
    case 'supplier':
      return (
        <>
          <ListGroup.Item>
            <strong>Product Type:</strong> {nodeData.productType}
          </ListGroup.Item>
          <ListGroup.Item>
            <strong>Delivery Schedule:</strong> {nodeData.deliverySchedule}
          </ListGroup.Item>
          <ListGroup.Item>
            <strong>Lead Time:</strong> {nodeData.leadTime} days
          </ListGroup.Item>
        </>
      );
    ...
    case 'process':
      return (
        <>
          <ListGroup.Item>
            <strong>Cycle Time:</strong> {nodeData.cycleTime} minutes
          </ListGroup.Item>
          <ListGroup.Item>
            <strong>Value-Added Time:</strong> {nodeData.valueAddedTime} minutes ({Math.round((nodeData.valueAddedTime / nodeData.cycleTime) * 100)}%)
          </ListGroup.Item>
          <ListGroup.Item>
            <strong>Non-Value-Added Time:</strong> {nodeData.nonValueAddedTime} minutes ({Math.round((nodeData.nonValueAddedTime / nodeData.cycleTime) * 100)}%)
          </ListGroup.Item>
          <ListGroup.Item>
            <strong>Staff:</strong> {nodeData.staff}
          </ListGroup.Item>
          {nodeData.steps && (
            <ListGroup.Item>
              ...
              <ul className="process-steps">
                {nodeData.steps.map((step, index) => (
                  <li key={index}>
                    {step.name} - {step.time} min
                    {step.accuracy && <span> (Accuracy: {step.accuracy}%)</span>}
