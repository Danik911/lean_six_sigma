...
const ValueStreamMap = ({ 
  ...
  useEffect(() => {
    ...
    data.processes.forEach(process => {
      // Apply what-if analysis adjustments if available
      let adjustedProcess = { ...process };
      
      if (whatIfSettings) {
        // Adjust based on staff training level
        if (process.id === "process-stock-take" && whatIfSettings.trainingLevel) {
          const accuracyModifier = whatIfSettings.trainingLevel === 'advanced' ? 1.2 :
                                  whatIfSettings.trainingLevel === 'intermediate' ? 1.1 : 1.0;
          
          const timeModifier = whatIfSettings.trainingLevel === 'advanced' ? 0.8 :
                              whatIfSettings.trainingLevel === 'intermediate' ? 0.9 : 1.0;
          
          adjustedProcess.valueAddedTime = Math.round(process.valueAddedTime * timeModifier);
          adjustedProcess.cycleTime = adjustedProcess.valueAddedTime + process.nonValueAddedTime;
          
          // Adjust steps if available
          if (adjustedProcess.steps) {
            adjustedProcess.steps = process.steps.map(step => {
              if (step.accuracy) {
                return {
                  ...step,
                  accuracy: Math.min(Math.round(step.accuracy * accuracyModifier), 99),
                  time: Math.round(step.time * timeModifier)
                };
              }
              return step;
            });
        ...
        // Adjust based on scanner model
        if ((process.id === "process-stock-take" || process.id === "process-receiving") && 
            whatIfSettings.scannerModel) {
          const scanTimeModifier = whatIfSettings.scannerModel === 'newModel' ? 0.7 :
                                  whatIfSettings.scannerModel === 'currentModel' ? 1.0 : 1.3;
          
          // Adjust scan time in steps if available
          if (adjustedProcess.steps) {
            adjustedProcess.steps = adjustedProcess.steps.map(step => {
              if (step.name.toLowerCase().includes('scan')) {
                return {
                  ...step,
                  time: Math.round(step.time * scanTimeModifier)
                };
              }
              return step;
            });
            ...
            const totalStepTime = adjustedProcess.steps.reduce((sum, step) => sum + step.time, 0);
            adjustedProcess.cycleTime = totalStepTime;
          }
        }
      }
      ...
      processesGroup.append("g")
        ...
        .on("click", () => handleProcessClick(adjustedProcess))
        .call(g => {
          g.append("rect")
            .attr("x", -60)
            .attr("y", -40)
          ...
          g.append("text")
            ...
            .text(`CT: ${adjustedProcess.cycleTime} min`);
          
          g.append("text")
            .attr("text-anchor", "middle")
            .attr("dy", "1.5em")
            .attr("class", "process-value")
            .text(`VA: ${Math.round((adjustedProcess.valueAddedTime / adjustedProcess.cycleTime) * 100)}%`);
          
          g.append("text")
            .attr("text-anchor", "middle")
            .attr("dy", "2.7em")
            .text(`Staff: ${adjustedProcess.staff}`);
          ...
          g.append("text")
            ...
            .text(() => {
              switch(selectedMetric) {
                case 'time':
                  return `Time: ${adjustedProcess.cycleTime} min`;
                case 'value':
                  return `Value: ${adjustedProcess.valueAddedTime} min`;
                case 'staff':
                  return `Staff: ${adjustedProcess.staff}`;
                case 'cost':
                  // Estimate cost based on staff and time
                  const hourlyRate = 25; // Assume €25/hour
                  const costPerMinute = hourlyRate / 60;
                  const processCost = Math.round(adjustedProcess.cycleTime * adjustedProcess.staff * costPerMinute);
                  return `Cost: €${processCost}`;
                default:
                  return `Time: ${adjustedProcess.cycleTime} min`;
              }
            });
  ...
  }, [data, view, selectedMetric, whatIfSettings, onNodeSelect]);
    ...
    <div className="value-stream-map">
      ...
      {selectedProcess && (
        <ProcessDetailModal 
          ...
          show={showProcessDetail}
          onHide={handleCloseProcessDetail}
        />
      )}
